...
// Guard: if Cognito is not configured, throw a clear error instead of
// a cryptic "Invalid URL" TypeError deep inside the PKCE flow.
function assertCognitoConfigured(): void {
  if (!COGNITO_DOMAIN || !CLIENT_ID) {
    throw new Error(
      'Cognito is not configured. ' +
      'Set VITE_COGNITO_DOMAIN and VITE_COGNITO_CLIENT_ID in your environment, ' +
      'or set VITE_USE_MOCK=true for offline dev.'
    );
  }
}
